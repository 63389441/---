...
import Secharts from '../components/Secahrts.vue';
...
export default {
    ...
    setup(props) {
        ...
        const DataLoad = () => {
            axios.get('../data/rel.json').then((Response) => {
                for (let i = 0, len = Response.data.length; i < len; i++) {
                    var relation = {
                        start: Response.data[i].p.properties[0].start.properties
                            .name,
                        end: Response.data[i].p.properties[0].end.properties.name,
                        relation:
                            Response.data[i].p.properties[0].relationship
                                .properties.name,
                        link: {
                            source: Response.data[i].p.properties[0].start
                                .identity,
                            target: Response.data[i].p.properties[0].end.identity,
                            value: Response.data[i].p.properties[0].relationship
                                .properties.name,
                        },
                    };
                    var NodesTemp = {
